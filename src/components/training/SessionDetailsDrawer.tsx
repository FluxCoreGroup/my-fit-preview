...
import { Timer, Dumbbell, CheckCircle2, AlertCircle, Clock } from "lucide-react";
...
export const SessionDetailsDrawer = ({
  open, onOpenChange, sessionName, estimatedTime, warmup, exercises, checklist, coachNotes,
          ...
          <div className="px-4 pb-8 overflow-y-auto space-y-4">
            {warmup.length > 0 && (
              <div className="p-4 bg-card/50 rounded-xl border border-primary/10">
                <h4 className="font-semibold mb-2 flex items-center gap-2 text-sm">
                  <Timer className="w-4 h-4 text-primary" />{t("session.warmup")}
                </h4>
            ...
            <div className="p-4 bg-card/50 rounded-xl border border-secondary/10">
              <h4 className="font-semibold mb-3 flex items-center gap-2 text-sm">
                <Dumbbell className="w-4 h-4 text-secondary" />{t("session.exercises")}
              </h4>
              <div className="space-y-2">
                {exercises.map((exercise, i) => (
                  <div key={i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 cursor-pointer transition-colors" onClick={() => setSelectedExercise(exercise)}>
                    <ExerciseImage exerciseName={exercise.name} englishName={exercise.englishName} size="sm" />
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-sm truncate">{exercise.name}</p>
                      <p className="text-xs text-muted-foreground">{exercise.sets}×{exercise.reps} • {exercise.rest}s {t("session.rest")}</p>
                    </div>
                    <Badge variant="secondary" className="text-xs">RPE {exercise.rpe}</Badge>
                  </div>
                ))}
              </div>
            </div>
            ...
            {checklist.length > 0 && (
              <div className="p-4 bg-card/50 rounded-xl border border-accent/10">
                <h4 className="font-semibold mb-2 flex items-center gap-2 text-sm">
                  <CheckCircle2 className="w-4 h-4 text-accent" />CHECKLIST
                </h4>
                <ul className="space-y-1 text-sm text-muted-foreground">
                  {checklist.map((item, i) => (
                    <li key={i} className="flex items-start gap-2"><span className="text-accent mt-0.5">☐</span><span>{item}</span></li>
                  ))}
                </ul>
              </div>
            )}
            ...
            {coachNotes && (
              <div className="p-4 bg-gradient-to-br from-primary/5 to-secondary/5 rounded-xl border border-primary/20">
                <h4 className="font-semibold mb-2 flex items-center gap-2 text-sm">
                  <AlertCircle className="w-4 h-4 text-primary" />{t("session.coachNotes")}
                </h4>
