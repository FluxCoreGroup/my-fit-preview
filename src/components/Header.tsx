...
export const Header = ({ variant = "marketing", showBack = false, onBack }: HeaderProps) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b bg-background/95 backdrop-blur-lg">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 font-bold text-xl">
          <Dumbbell className="w-6 h-6 text-primary" />
          <span>Pulse.ai</span>
        ...
        <nav className="hidden md:flex items-center gap-6">
          <a href="#evaluation" className="text-sm text-muted-foreground hover:text-foreground transition-colors">
            Évaluation
          </a>
