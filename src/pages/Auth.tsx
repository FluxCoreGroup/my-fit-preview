...
const Auth = () => {
  ...
  const loginSchema = z.object({
    email: z.string().email(t("login.email")).max(255),
    password: z.string().min(6, t("login.password")),
  });
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    if (!isPasswordValid) {
      toast({
        title: t("login.passwordInvalid"),
        description: t("login.passwordInvalidDesc"),
        variant: "destructive",
      });
      return;
    }
        ...
        <div className="max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">
              {isRecoveryMode ? t("login.recoverySubtitle") : t("login.subtitle")}
            </p>
          </div>
              ...
              <form onSubmit={handleUpdatePassword} className="space-y-4">
                <div>
                  ...
                  <div className="relative mt-2">
                    ...
                    <Input id="new-password" type={showNewPassword ? "text" : "password"} placeholder="••••••••" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="pl-10 pr-10" required />
                ...
                <div className="space-y-1 py-2">
                  <PasswordCriterion met={hasMinLength} text={t("login.minChars")} />
                  <PasswordCriterion met={hasUppercase} text={t("login.uppercase")} />
                  <PasswordCriterion met={hasNumber} text={t("login.number")} />
                </div>
                <div>
                  <Label htmlFor="confirm-password">{t("login.confirmPassword")}</Label>
                  <div className="relative mt-2">
                    <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                    <Input id="confirm-password" type={showConfirmPassword ? "text" : "password"} placeholder="••••••••" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="pl-10 pr-10" required />
                  ...
                  {confirmPassword.length > 0 && (
                    <p className={`text-sm mt-1 ${passwordsMatch ? "text-green-500" : "text-destructive"}`}>
                      {passwordsMatch ? t("login.passwordsMatch") : t("login.passwordsDontMatch")}
                    </p>
                  )}
                </div>
                <Button type="submit" size="lg" className="w-full" disabled={loading || !isPasswordValid}>
                  {loading ? t("login.updateLoading") : t("login.updatePassword")}
                </Button>
              </form>
            ) : (
                  ...
                  <>
                    <div>
                      ...
                      <div className="relative mt-2">
                        ...
                        <Input id="login-email" type="email" placeholder={t("login.emailPlaceholder")} value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} className="pl-10" required />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="relative mt-2">
                        ...
                        <button type="button" onClick={() => setShowLoginPassword(!showLoginPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground">
                    ...
                    <p className="text-sm text-center text-muted-foreground pt-4 border-t">
                      {t("login.noAccount")}{" "}
                      <Link to="/start" className="font-semibold text-primary hover:underline">{t("login.startQuestionnaire")}</Link>
                    </p>
                  </>
                ) : (
                  <>
                    <div>
                      ...
                      <div className="relative mt-2">
                        <Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                        <Input id="reset-email" type="email" placeholder={t("login.emailPlaceholder")} value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} className="pl-10" required />
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Button type="button" variant="outline" onClick={() => setShowResetPassword(false)} className="w-full">{t("login.back")}</Button>
                      <Button type="button" onClick={handleResetPassword} className="w-full" disabled={loading}>
                        {loading ? t("login.sendLoading") : t("login.resetButton")}
                      </Button>
                    </div>
                  </>
                )}
