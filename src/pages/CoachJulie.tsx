...
const CoachJulie = () => {
  ...
  const julieContext = {
    goal_type: goals?.goal_type, tdee: goals?.weight && goals?.height && goals?.age ? "calculé" : "non calculé",
    target_calories: "à calculer", protein: goals?.weight ? Math.round(goals.weight * 2) : 0,
    fat: goals?.weight ? Math.round(goals.weight * 1) : 0, carbs: 0,
    meals_per_day: goals?.meals_per_day, restrictions: goals?.restrictions, allergies: goals?.allergies,
  };

  const julieShortcuts = t("julieShortcuts", { returnObjects: true }) as string[];

  const conversationListComponent = (
    <ConversationList activeConversationId={activeConversationId} onSelectConversation={(id) => { setActiveConversationId(id); if (isMobile) setIsConversationListOpen(false); }} coachType="julie" />
  );
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-16 flex flex-col md:flex-row h-[calc(100vh-4rem)]">
            ...
            <div className="flex items-center justify-between p-3 border-b bg-card/50 backdrop-blur-sm">
              ...
              <div className="flex items-center gap-2">
                <div className="relative">
                  <img src={coachJulieAvatar} alt="Julie" className="w-9 h-9 rounded-full object-cover ring-2 ring-border" />
        ...
        <div className="flex-1 flex flex-col">
          {!isMobile && <CoachHeader name="Julie" role={t("julieRole")} avatar={coachJulieAvatar} isOnline={true} />}
          {goalsLoading ? (
            <div className="p-4"><ChatSkeleton /></div>
          ) : (
            <ChatInterface conversationId={activeConversationId} conversation={conversations.find(c => c.id === activeConversationId)} functionName="chat-julie" systemPrompt="" shortcuts={julieShortcuts} context={julieContext} avatarColor="bg-secondary" name="Julie" coachType="julie" onConversationCreated={setActiveConversationId} />
          )}
        </div>
      </div>
    </div>
