...
import { Dumbbell, Settings as SettingsIcon, AlertCircle, TrendingUp, ChevronDown, BarChart3, Loader2 } from "lucide-react";
...
const Training = () => {
  ...
  // Track if auto-generation has been attempted to prevent multiple calls
  ...
  // Auto-generate first program for new users
  useEffect(() => {
    const autoGenerate = async () => {
      // Skip if already attempted, loading, or generating
      if (hasAutoGenerated.current || loading || isGenerating) return;
      
      // Only auto-generate for first-time users (week 0, no sessions, no history)
      if (currentWeek !== 0) return;
      if (sessions.length > 0) return;
      if (historicalPrograms.length > 0) return;
      
      // Check if generation is allowed
      const { allowed } = await canGenerateWeek();
      if (allowed) {
        hasAutoGenerated.current = true;
        console.log("Auto-generating first weekly program...");
        await generateWeeklyProgram(false);
      }
    };

    autoGenerate();
  }, [loading, currentWeek, sessions, historicalPrograms, canGenerateWeek, generateWeeklyProgram, isGenerating]);
